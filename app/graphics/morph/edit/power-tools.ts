...
export const GlobalFieldTransformMorph = createMorph<EditOutput, EditOutput>(
  ...
  (shape, context) => {
    ...
    const { fieldPattern, transform, options } = context.powerTools?.globalTransform || {};
    
    // Skip if no transformation defined
    if (!fieldPattern || !transform) return shape;
    
    // Create regex from pattern if string
    const pattern = typeof fieldPattern === 'string' ? new RegExp(fieldPattern) : fieldPattern;
      ...
      meta: {
        ...
        powerTools: {
          ...
          lastTransform: {
            ...
            fieldsAffected: transformedFields.filter((f, i) => f !== shape.fields[i]).length
          }
        }
      }
    };
...
export const QueryFilterFieldsMorph = createMorph<EditOutput, EditOutput>(
  ...
  (shape, context) => {
    ...
    const { query, visible, highlight } = context.powerTools?.fieldQuery || {};
    ...
    const queryResults = shape.fields.map(field => {
      ...
      const matches = typeof query === 'function' 
        ? query(field, shape)
        : Object.entries(query).every(([key, value]) => {
            const fieldValue = key.split('.').reduce((obj, path) => obj?.[path], field);
            return value instanceof RegExp ? value.test(fieldValue) : fieldValue === value;
          });
...
export const BatchOperationsMorph = createMorph<EditOutput, EditOutput>(
  ...
  (shape, context) => {
    ...
    const { operations, selection } = context.powerTools?.batch || {};
...
export const ApplyTemplateMorph = createMorph<EditOutput, EditOutput>(
  ...
  (shape, context) => {
    ...
    const { template, target, data } = context.powerTools?.template || {};
    ...
    const applyFieldTemplate = (field: any, templateData: any) => {
      ...
      const applyStringTemplate = (str: string) => {
        return str.replace(/\{\{([^}]+)\}\}/g, (match, key) => {
          return key.split('.').reduce((obj, path) => obj?.[path] ?? match, templateData);
        });
      };
    ...
    if (target === 'allFields') {
      ...
      return {
        ...shape,
        fields: shape.fields.map(field => applyFieldTemplate(field, { ...data, field }))
      };
    } else if (target === 'selectedFields' && Array.isArray(context.powerTools?.template?.fields)) {
      // Apply to selected fields
      const templateFields = new Set(context.powerTools.template.fields);
          ...
          sections: shape.layout.sections.map(section => ({
            ...
            title: typeof section.title === 'string' 
              ? section.title.replace(/\{\{([^}]+)\}\}/g, (match, key) => {
                  return key.split('.').reduce((obj, path) => obj?.[path] ?? match, { ...data, section });
                })
              : section.title,
            meta: section.meta ? {
              ...section.meta,
...
export const KBRefactorMorph = createMorph<EditOutput, EditOutput>(
  ...
  (shape, context) => {
    ...
    const { operations } = context.powerTools?.refactor || {};
        ...
        switch (operation.type) {
          case 'renameField':
            if (operation.oldId && operation.newId) {
              ...
              const fieldIndex = resultShape.fields.findIndex(f => f.id === operation.oldId);
              ...
              if (fieldIndex >= 0) {
                ...
                if (layout?.sections) {
                  layout = {
                    ...
                    sections: layout.sections.map(section => ({
                      ...section,
                      fields: section.fields.map(id => id === operation.oldId ? operation.newId : id)
                    }))
                  };
                }
                ...
                operationLog.details = { field: operation.oldId, newId: operation.newId };
              } else {
          ...
          case 'moveField':
            if (operation.fieldId && operation.targetSection) {
              ...
              const field = resultShape.fields.find(f => f.id === operation.fieldId);
              let layout = resultShape.layout;
              ...
              if (field && layout?.sections) {
                ...
                const updatedSections = layout.sections.map(section => ({
                  ...section,
                  fields: section.fields.filter(id => id !== operation.fieldId)
                }));
                
                // Add field to target section
                const targetSectionIndex = updatedSections.findIndex(
                  s => s.id === operation.targetSection || s.title === operation.targetSection
                );
                
                if (targetSectionIndex >= 0) {
                  updatedSections[targetSectionIndex] = {
                    ...updatedSections[targetSectionIndex],
                    fields: [...updatedSections[targetSectionIndex].fields, operation.fieldId]
                  };
          ...
          case 'splitField':
            if (operation.sourceId && operation.targetFields) {
              ...
              const sourceIndex = resultShape.fields.findIndex(f => f.id === operation.sourceId);
              ...
              if (sourceIndex >= 0) {
                ...
                const newFields = operation.targetFields.map((target: any) => ({
                  ...
                  value: typeof target.extractor === 'function' 
          ...
          case 'mergeFields':
            if (operation.sourceIds && operation.targetId && operation.merger) {
              ...
              const sourceFields = resultShape.fields.filter(f => 
                f.id && operation.sourceIds.includes(f.id)
              );
              ...
              if (sourceFields.length === operation.sourceIds.length) {
                ...
                const mergedField = {
                  ...
                  meta: {
                    ...
                    originalFields: sourceFields.map(f => ({ id: f.id, value: f.value }))
                  }
                };
                
                // Remove source fields and add merged field
                const fields = resultShape.fields.filter(f => 
                  !(f.id && operation.sourceIds.includes(f.id))
                );
...
export const KnowledgeGraphMorph = createMorph<EditOutput, EditOutput>(
  ...
  (shape, context) => {
    ...
    const { operations } = context.powerTools?.graph || {};
        ...
        switch (operation.type) {
          ...
          case 'removeRelationship':
            if (operation.id) {
              ...
              relationships = relationships.filter(r => r.id !== operation.id);
              ...
              relationships = relationships.filter(r => 
                !(r.source === operation.source && r.target === operation.target &&
                  (!operation.relationType || r.type === operation.relationType))
              );
          ...
          case 'updateRelationship':
            if (operation.id && operation.updates) {
              ...
              const relationshipIndex = relationships.findIndex(r => r.id === operation.id);
...
export const StyleFormattingMorph = createMorph<EditOutput, EditOutput>(
  ...
  (shape, context) => {
    ...
    const { styleRules, scope } = context.powerTools?.style || {};
    ...
    const styledFields = shape.fields.map(field => {
      ...
      Object.entries(styleRules).forEach(([styleKey, styleValue]) => {
        switch (styleKey) {
          case 'labelCase':
            if (fieldWithStyle.label && typeof fieldWithStyle.label === 'string') {
              switch (styleValue) {
                case 'title':
                  ...
                  break;
                case 'sentence':
                  fieldWithStyle.label = fieldWithStyle.label.charAt(0).toUpperCase() + 
...
export const KBPowerToolsPipeline = createPipeline<EditOutput, EditOutput>("KBPowerToolsPipeline")
