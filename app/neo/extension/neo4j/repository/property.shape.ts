 ...
 */
export class PropertyShapeRepository { // Renamed class
  ...
  async saveProperty(property: FormProperty): Promise<FormProperty> {
    ...
    try {
      ...
      `, {
        ...
        hasStaticValue: property.staticValue !== undefined && property.staticValue !== null,
        staticValue: property.staticValue !== undefined && property.staticValue !== null ?
        ...
        min: property.quantitative?.range?.min !== undefined ?
             (typeof property.quantitative.range.min === 'object' ? JSON.stringify(property.quantitative.range.min) : property.quantitative.range.min) :
             null,
        max: property.quantitative?.range?.max !== undefined ?
             (typeof property.quantitative.range.max === 'object' ? JSON.stringify(property.quantitative.range.max) : property.quantitative.range.max) :
             null,
      });
      ...
      await txc.rollback(); // Rollback might be needed explicitly here
      throw error;
    } finally {
      await session.close();
    }
  ...
  async getPropertyById(id: string): Promise<FormProperty | null> {
    ...
    try {
      ...
      if (prop.qualitative_essential !== undefined && prop.qualitative_essential !== null) qualitative.essential = prop.qualitative_essential;
      if (prop.qualitative_observable !== undefined && prop.qualitative_observable !== null) qualitative.observable = prop.qualitative_observable;
      if (prop.qualitative_mutable !== undefined && prop.qualitative_mutable !== null) qualitative.mutable = prop.qualitative_mutable;
      if (prop.qualitative_inherent !== undefined && prop.qualitative_inherent !== null) qualitative.inherent = prop.qualitative_inherent;

      // Build quantitative object
      const quantitative: any = {};
      if (prop.quantitative_dataType !== undefined && prop.quantitative_dataType !== null) quantitative.dataType = prop.quantitative_dataType;
      if (prop.quantitative_unit !== undefined && prop.quantitative_unit !== null) quantitative.unit = prop.quantitative_unit;
      if (prop.quantitative_precision !== undefined && prop.quantitative_precision !== null) {
         // Neo4j might return Integer or Float, ensure it's a number
         quantitative.precision = typeof prop.quantitative_precision === 'number'
   ...
   */
  async findPropertiesByFormContext(contextId: string): Promise<FormProperty[]> {
    const session = this.connection.getSession({ defaultAccessMode: 'READ' });
    ...
    try {
      ...
      for (const record of result.records) {
         ...
         const prop = record.get('p').properties;
         let staticValue = prop.staticValue;
         try { if (typeof staticValue === 'string' && (staticValue.startsWith('{') || staticValue.startsWith('['))) { staticValue = JSON.parse(staticValue); } } catch (e) {}
         const qualitative: any = {};
         if (prop.qualitative_essential !== undefined && prop.qualitative_essential !== null) qualitative.essential = prop.qualitative_essential;
         ...
         properties.push({
            ...
            createdAt: prop.createdAt ? new Date(prop.createdAt.toString()).getTime() : Date.now(),
            updatedAt: prop.updatedAt ? new Date(prop.updatedAt.toString()).getTime() : Date.now()
         });
      }
      return properties;
    } catch (error) {
      console.error(`Error finding property shapes by context in Neo4j: ${error}`);
      throw error;
    } finally {
      await session.close();
    }
  ...
  async findPropertiesByFormEntity(entityId: string): Promise<FormProperty[]> {
    ...
    try {
      ...
      console.error(`Error finding property shapes by entity in Neo4j: ${error}`);
      throw error;
    } finally {
      await session.close();
    }
