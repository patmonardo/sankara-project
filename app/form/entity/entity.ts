...
export class NeoEntityService {
  ...
  private static entities: Map<string, NeoEntity> = new Map();
  ...
  static create(options: {
    ...
    return new NeoEntityService(options);
  }
  
  /**
   * Constructor for NeoEntity
   */
  constructor(options: {
    id?: string;
    type?: string;
   ...
   */
  static registerEntity(entity: NeoEntity): void {
    NeoEntityService.entities.set(entity.id, entity);
  }
   ...
   * Moved from NeoCore
   */
  static createEntity(data: Partial<NeoEntity>): string {
    const id = data.id || `entity:${Date.now()}:${Math.random().toString(36).substring(2, 9)}`;
    
    const newEntity = new NeoEntityService({
      id,
    ...
    // Register the entity
    NeoEntityService.entities.set(id, newEntity);
   ...
   * Moved from NeoCore
   */
  static getEntity(id: string): NeoEntity | undefined {
    return NeoEntityService.entities.get(id);
  }
   ...
   * Moved from NeoCore
   */
  static updateEntity(id: string, data: Partial<NeoEntity>): boolean {
    const entity = NeoEntityService.entities.get(id);
    if (!entity) {
      return false;
    }
    ...
    NeoEntityService.entities.set(id, entity);
  ...
  static removeEntity(id: string): boolean {
    ...
    // Remove the entity
    NeoEntityService.entities.delete(id);
  ...
  static queryEntities(filter: {
    ...
    return Array.from(NeoEntityService.entities.values()).filter(entity => {
  ...
  persist(): string {
    ...
    if (NeoEntityService.getEntity(this.id)) {
      NeoEntityService.updateEntity(this.id, entityData);
    } else {
      NeoEntityService.createEntity(entityData);
    }
   ...
   */
  delete(): boolean {
    return NeoEntityService.removeEntity(this.id);
  }
...
/**
 * Create a NeoEntity instance
 * 
 * This function creates a NeoEntity with the provided configuration,
 * making it available for import by other modules.
 */
export function createNeoEntity(config: {
  id?: string;
  type?: string;
  properties?: Record<string, any>;
  metadata?: Record<string, any>;
  ...
  NeoEntityService.registerEntity(entity);
