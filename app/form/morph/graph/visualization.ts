...
>(
  ...
  (graph, context) => {
    ...
    if (
      context?.data?.config?.operation === "create" ||
    ...
    const focusAreas =
      options.highlightCommunities && graph.analysis?.communities
        ? generateFocusAreas(graph.analysis.communities)
        : undefined;
...
function generateStyling(graph: GraphShape): {
  ...
  Array.from(uniqueTypes).forEach((type, index) => {
    const color = relationColors[index % relationColors.length];
